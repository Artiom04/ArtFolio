...
namespace ArtFolio.GestioneAccount.Control;
...
internal class GestioneAccountControl
{
	...
	public void cliccaEliminaAccount()
	{
		...
		if (passwordStudente == null) return;
		var b = DBMSBoundary.get();
	...
	{
		var mittente = new MailAddress("coniglio.ert2468@gmail.com", "ArtFolio");
		var destinatario = new MailAddress(email, "");

		const string password = "PASSWORD";

		const string oggetto = "Codice accesso ArtFolio";
		string corpo = $"il codice di accesso è {codice}";
		...
		var smtp = new SmtpClient
		{
			...
			Credentials = new NetworkCredential(mittente.Address, password),
		};

		using (var messaggio = new MailMessage(mittente, destinatario)
		{
			Subject = oggetto,
			Body = corpo,
			IsBodyHtml = false
	...
	public void cliccaRegistrati()
	{
		...
		var credenzialiVerificate = verificaFormatoCredenziali(new string[] { credenziali["nickname"], credenziali["password"] });
		var msg = new MessageBoundary(); 
