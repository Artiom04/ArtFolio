...
internal class DBMSBoundary
{
	...
	public Dictionary<string, object> getDatiStudente(int studenteID)
	{
		...
		try
		{
			...
			{
				cmd = new SqlCommand("SELECT blob_materiale FROM materiale WHERE materiale_id = @materiale_id", connection);
		...
		try
		{
			...
			while (postList.Count > 0)
			{
				...
				while (materialiList.Count > 0)
				{
					...
					//TODO
					if (nomeMateriale.EndsWith(".png") || nomeMateriale.EndsWith(".jpg") || nomeMateriale.EndsWith(".jpeg"))
					{
						result.Add(getMateriale(materialeID));
					}
					else
